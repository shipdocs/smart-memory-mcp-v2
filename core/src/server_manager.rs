                    ...
                    stdout.lines().any(|line| {
                        line.contains(&format!(":{}", self.port))
                            ...
                            && line
                                .split_whitespace()
                                .last()
                                .map_or(false, |s| s == pid.to_string())
                    })
                }
                Err(_) => false,
            }
                            ...
                            if let Ok(manager) = ServerManager::new() {
                                if let Some(server_pid) = manager.is_server_running() {
                                    if manager.stop_server(server_pid) {
                                        println!(
                                            "Successfully stopped server with PID {}",
                                            server_pid
                                        );
                                    }
                                }
        ...
        "backup" => {
            ...
            match crate::storage::BackupManager::new(&backup_dir) {
                Ok(backup_manager) => {
                    ...
                    let description = args.get(2).map(|s| s.as_str()).unwrap_or("Manual backup");

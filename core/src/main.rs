...
mod proto {
    ...
    // Include the generated descriptor set for reflection
    pub const FILE_DESCRIPTOR_SET: &[u8] = include_bytes!("proto/smart_memory.bin");
}

use crate::logging::LogLevel;
use crash_recovery::CrashRecoveryManager;
use parent_process_monitor::{
    is_shutdown_requested, start_parent_process_monitor, wait_for_shutdown_request,
};
...
async fn main() -> Result<()> {
    ...
    let mut recovery_manager = match CrashRecoveryManager::new(&data_path) {
        ...
        Err(e) => {
            log_error!(
                "main",
                &format!("Failed to initialize crash recovery system: {}", e)
            );
    ...
    let version_manager = match version::init() {
        Ok(manager) => {
            log_info!(
                "main",
                &format!(
                    "Version manager initialized. Current version: {}",
        ...
        Err(e) => {
            log_error!(
                "main",
                &format!("Failed to initialize version manager: {}", e)
            );
    ...
    if let Err(e) = recovery_manager.set_paths(&db_path, &config_path, port) {
        log_error!(
            "main",
            &format!("Failed to update crash recovery paths: {}", e)
        );
    }
    ...
    if db_path_buf.exists() {
        match storage::BackupManager::new(&backup_dir) {
            ...
            Err(e) => {
                log_warning!(
                    "main",
                    &format!("Failed to initialize backup manager: {}", e)
                );
            }
        }
    } else {
    ...
    log_info!(
        "main",
        &format!(
            "[{}ms] Memory store created successfully",
            start_time.elapsed().as_millis()
        )
    );
    ...
    log_info!(
        "main",
        &format!(
            "[{}ms] Memory service created successfully",
            start_time.elapsed().as_millis()
        )
    );
    ...
    log_info!(
        "main",
        &format!(
            "[{}ms] Health service created successfully",
            start_time.elapsed().as_millis()
        )
    );
    ...
    log_info!(
        "main",
        &format!(
            "[{}ms] Server configured, starting to serve...",
            start_time.elapsed().as_millis()
        )
    );
    ...
    if parent_monitor_handle.is_some() {
        ...
        log_warning!(
            "main",
            "Failed to start parent process monitor, VSCode process monitoring disabled"
        );
    }
    ...
    if let Some(handle) = parent_monitor_handle {
        if let Err(e) = handle.join() {
            log_error!(
                "main",
                &format!("Failed to join parent monitor thread: {:?}", e)
            );
        }
    }

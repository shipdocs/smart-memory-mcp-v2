...
impl BackupManager {
    ...
    pub fn create_backup(&self, source_path: &Path, description: &str) -> io::Result<PathBuf> {
        ...
        log_info!(
            "backup",
            &format!("Created backup: {}", backup_path.display())
        );
    ...
    pub fn restore_backup(&self, backup_path: &Path, target_path: &Path) -> io::Result<()> {
        ...
        if target_path.exists() {
            ...
            match self.create_backup(target_path, &pre_restore_desc) {
                Ok(path) => log_info!(
                    "backup",
                    &format!("Created pre-restore backup: {}", path.display())
                ),
                Err(e) => log_warning!(
                    "backup",
                    &format!("Failed to create pre-restore backup: {}", e)
                ),
            }
        }
        ...
        if target_path.exists() {
            log_info!(
                "backup",
                &format!(
                    "Removing target file before restore: {}",
                    target_path.display()
                )
            );
        ...
        if restored_content != original_content {
            log_error!(
                "backup",
                "Restored content does not match original backup content"
            );
            return Err(io::Error::new(
                io::ErrorKind::Other,
    ...
    pub fn delete_backup(&self, backup_path: &Path) -> io::Result<()> {
        ...
        log_info!(
            "backup",
            &format!("Deleted backup: {}", backup_path.display())
        );
    ...
    fn rotate_backups(&self) -> io::Result<()> {
        ...
        log_info!(
            "backup",
            &format!("Backups before rotation: {}", backups.len())
        );
        log_info!(
            "backup",
            &format!("Max backups allowed: {}", self.max_backups)
        );
        ...
        if backups.len() > self.max_backups {
            ...
            for (path, _) in backups.iter().rev().take(to_delete) {
                log_info!(
                    "backup",
                    &format!("Deleting old backup: {}", path.display())
                );
                if let Err(e) = self.delete_backup(path) {
                    log_warning!(
                        "backup",
                        &format!("Failed to delete old backup {}: {}", path.display(), e)
                    );
                }
            }
        }
